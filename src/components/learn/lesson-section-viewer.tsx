...
function YouTubePlayer({ ytId, title }: { ytId: string; title?: string }) {
  ...
  return (
    <div className="w-full space-y-2">
      ...
      <div
        className="border-default relative overflow-hidden rounded-xl border shadow-sm"
        style={{ paddingBottom: "56.25%" }}
      >
        {!playing ? (
          <button
...
function VideoRenderer({ url, title }: { url: string; title?: string }) {
  ...
  return (
    <div className="w-full space-y-2">
      ...
      <video
        controls
        className="border-default w-full rounded-xl border shadow-sm"
        src={url}
      >
        Your browser does not support the video tag.
      </video>
  ...
  switch (section.type) {
        ...
        >
          {section.url ? (
            <Image
              ...
              width={1200}
              height={800}
              className={`${currentSizeCls} ${getAlignClass(section.align)} border-default bg-checkerboard max-h-[75vh] rounded-xl border object-contain`}
              loading="lazy"
            />
          ) : (
            <div
              className={`bg-subtle border-default text-muted flex h-32 items-center justify-center rounded-xl border border-dashed text-xs ${currentSizeCls}`}
            >
              Image section
            </div>
          )}
    ...
    case "pdf":
      return (
        ...
        >
          <div className="border-default bg-surface flex w-full max-w-[260px] items-center gap-2 rounded-lg border px-3 py-2 shadow-sm">
      ...
      return (
        ...
        >
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="group border-default bg-surface hover:border-primary flex w-full max-w-3xl items-start gap-4 rounded-xl border px-3 py-3 shadow-sm transition-all hover:shadow-sm"
          >
    ...
    case "task":
      return (
        ...
        >
          <div
            className={`max-w-2xl space-y-4 rounded-xl border border-indigo-100 bg-white p-5 shadow-sm ${getAlignClass(section.align)}`}
          >
            ...
            <div className="pt-2">
              <button
                type="button"
                className="w-full rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-indigo-700 sm:w-auto"
              >
        ...
        >
          {cols.map((col: any) => (
            <div key={col.id} className="min-w-0">
              ...
              {col.type === "image" && !col.url && (
                <div className="text-muted bg-subtle border-default m-3 flex h-24 items-center justify-center rounded-lg border border-dashed text-xs">
                  No image
                </div>
              )}
              ...
              {col.type === "pdf" && (
                <div className="p-3">
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="bg-surface flex items-center gap-3 rounded-lg px-3 py-2.5 transition-all hover:shadow-sm"
                  >
                    <div className="flex size-8 shrink-0 items-center justify-center rounded-md border border-red-100 bg-red-50">
                      <FileText className="size-4 text-red-500" />
                    </div>
              ...
              {col.type === "link" && (
                <div className="p-3">
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="group bg-surface flex items-start gap-3 rounded-lg px-3 py-2.5 transition-all hover:shadow-sm"
                  >
                    <div className="mt-0.5 flex size-12 shrink-0 items-center justify-center overflow-hidden rounded-xl border border-teal-100 bg-teal-50 p-1.5">
                      {col.thumbnailUrl ? (
                        <Image
              ...
              {!col.type && (
                <div className="text-muted bg-subtle border-default m-3 flex h-16 items-center justify-center rounded-lg border border-dashed text-xs">
                  Empty column
                </div>
              )}
            </div>
